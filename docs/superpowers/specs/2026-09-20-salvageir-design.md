...
建立一条单一 `TRANSLATOR` 研究主线：对可解析但被 Alive2 明确反驳的 LLM LLVM-IR 优化候选进行 refinement-guided partial rollback。核心系统将源—目标差异构造成依赖闭合编辑组件，以反例切片排序回滚集合，在获得 verified 状态后重新加入低风险组件恢复性能，并对最终完整函数执行直接验证。
...
Frozen Candidate
  -> Normalizer/Aligner
  -> Dependency-Closed Component Graph
  -> Counterexample Slicer
  -> Budgeted Rollback Search
  -> Profit Recovery
